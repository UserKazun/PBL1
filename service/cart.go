...
// DeleteCartContent ...カートの中身を全て削除する
func AllDeleteCartContent() error {
	carts := []model.Cart{}
...
// DeleteCartContentByUserIDandRecipeID ...ユーザーIDとレシピIDで削除するレシピを決める
func DeleteCartContentByUserIDandRecipeID(userID string, recipeID uint) error {
	carts := []model.Cart{}

	err := db.Where("user_id = ? and recipe_id = ?", userID, recipeID).Delete(&carts).Error
	if err != nil {
		return err
	}
